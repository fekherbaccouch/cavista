    ...
    <Link href={`/product/${product.slug || product.id}`} className="block group">
      <div className="bg-surface border border-border/60 rounded-2xl overflow-hidden transition-all duration-500 group-hover:border-gold/30 group-hover:shadow-[0_8px_40px_rgba(0,0,0,0.5),0_0_0_1px_rgba(212,168,67,0.1)]">
        ...
        <div className="relative aspect-square overflow-hidden bg-surface2">
          <img
            src={product.image || "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&q=80"}
            alt={product.name}
          ...
          {product.badge && (
            <div className="absolute top-3 left-3 z-10">
              {product.badge === "promo" && discount ? (
                <span className="text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-gold text-bg">
                  -{discount}%
                </span>
              ) : product.badge === "new" ? (
                <span className="text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-emerald text-white">
                  Nouveau
                </span>
              ) : null}
            </div>
        ...
        <div className="p-4 space-y-2.5">
          {product.category && (
            <span className="text-[10px] uppercase tracking-[0.2em] text-gold font-semibold capitalize">
              {product.category}
            </span>
          )}
